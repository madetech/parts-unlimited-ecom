...
describe ViewSummary do
  ...
  let(:view_summary) { described_class.new(customer_gateway: file_customer_gateway, items_gateway: file_items_gateway, calculate_total_cost: calculate_total_cost) }
  ...
  it 'uses the gateway to retrieve the details' do
    ...
    view_summary = described_class.new(customer_gateway: customer_double , items_gateway: spy, calculate_total_cost: spy)
  ...
  it 'returns items' do
    view_summary = described_class.new(customer_gateway: spy, items_gateway: double(all: [Stub.new('113', 'Billy Bob', '12', '2', '24')]), calculate_total_cost: spy)
    response = view_summary.execute
    expect(response[:items]).to eq([{id:'113', name:'Billy Bob', price:'12', quantity:'2', total:'24.00'}])
  end

  it 'returns net total' do
    view_summary = described_class.new(customer_gateway: spy, items_gateway: spy, calculate_total_cost: double(execute: '50.00'))
    response = view_summary.execute
